...
#define BUTTON_PIN2 12 // Pin 12 of GPIOC
#define BUTTON_PIN3 10 // Pin 14 of GPIOC

#define BUTTON_IS_PRESSED()    (!(GPIOC->IDR & (1 << BUTTON_PIN)))
#define BUTTON_IS_RELEASED()   (GPIOC->IDR & (1 << BUTTON_PIN))
#define BUTTON_IS_PRESSED2()    (!(GPIOC->IDR & (1 << BUTTON_PIN2)))
#define BUTTON_IS_RELEASED2()   (GPIOC->IDR & (1 << BUTTON_PIN2))
#define BUTTON_IS_PRESSED3()    (!(GPIOC->IDR & (1 << BUTTON_PIN3)))
...
void configure_gpio(void)
{
    ...
    *RCC_APB2ENR |= (1 << 0); // RCC_APB2ENR_SYSCFGEN
    ...
    SYSCFG->EXTICR[3] &= ~(0xF << 0); // Limpiar bits para EXTI12
    SYSCFG->EXTICR[3] |= (0x2 << 0);  // Mapear EXTI12 al puerto GPIOC
    SYSCFG->EXTICR[2] &= ~(0xF << 8);  // Limpiar los bits [11:8] para EXTI10
    SYSCFG->EXTICR[2] |= (0x2 << 8);   // Mapear EXTI10 al puerto GPIOC
    ...
    // Configure EXTI14 for falling edge trigger
